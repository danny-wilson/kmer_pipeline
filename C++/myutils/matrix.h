...
namespace myutils
{
	...
	/*template <typename T>
	class AbstractMatrix	// Not yet used as base class
	{
	public:
		...
		virtual const T* operator[](int pos) = 0;
		// Matrix operations
		virtual AbstractMatrix<T> operator*(const AbstractMatrix<T>& mat) = 0;
		virtual AbstractMatrix<T>& multiply(const Matrix<T>& op1, const Matrix<T>& op2) = 0;
		virtual AbstractMatrix<T> map(T (* f)(T)) = 0;
		virtual AbstractMatrix<T> invert() = 0;
	}*/
