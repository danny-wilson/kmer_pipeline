// Read a list of kmer count files, case control status and min kmer count and test for association
...
int main(const int argc, const char* argv[]) {
  if(argc!=3) error("Usage: gwas_kmer_pattern_variable_mincount kmer-pheno-mincount.txt out-prefix");
  ...
  for(i=0;!infile.eof();i++) {
    ...
    infile >> mincount_in;
    if(!infile.good()) {
      cout << "Minimum kmer coverage in " << filename << " line " << i+1 << " was not an integer" << endl;
      error("");
    }
    ...
    // Test whether the minimum kmer count is valid
    if(mincount_in<1) error("The min-coverage must be positive");
    if(mincount_in>10000) error("The min-coverage cannot exceed 10000");
    mincount.push_back(mincount_in);
  }
  ...
  if(n>MAXN) {
    cout << "The maximum number of genomes under the current compilation is " << MAXN << endl;
  ...
  for(;;nkmers++) {
    for(i=0;i<n;i++) {
      if(next_kmer[i]==smallest_kmer && next_kmer_count[i]>=mincount[i]) {
