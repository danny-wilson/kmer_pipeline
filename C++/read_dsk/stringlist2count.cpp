// Read a gzipped list of kmers and a list of gzipped kmer count files and sum the sample counts for each listed kmer
...
int main(const int argc, const char* argv[]) {
	if(argc!=4 && argc!=6) error("Usage: stringlist2count sorted-kmer-list.txt.gz kmer-sorted-gzfile-list.txt out-prefix [k min-coverage]");
	...
	zstr::ofstream countOut((string(out_prefix)+".count.txt.gz").c_str());
  
	// Create storage for the counts
	vector< unsigned int > count(nkmers,0);
			...
			if(next_kmer_count>=mincount) {
                if(next_kmer<smallest_target) {
                    ...
                    if(next_found_kmer_it==kmer_list.end()) {
						...
						// Increment the count for that kmer
						count[pos]++;
    ...
    cout << "Completed reading in kmers" << endl;
	
	// Output the count for all input strings
    vector< unsigned int >::const_iterator kpatit;
	for(kpatit=count.begin();kpatit!=count.end();kpatit++) {
		countOut << *kpatit << endl;
	}
    cout << "Completed writing counts" << endl;
