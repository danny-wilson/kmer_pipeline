...
"Daniel Wilson (2018)",
"",
"Usage: qsub -t 1:p kmermerge.Rscript n p output_prefix input_dir output_dir id_file kmer_length process",
sep="\n")
...
while(TRUE) {
  ...
  if(i==1) {
    ...
    if(length(infiles)==1) {
      cmd = paste0("zcat ",infiles[1]," | cut -d \" \" -f1 > ",outfile) # Changed cut delimiter
    } else if(length(infiles)==2) {
      cmd = paste0("sort -um <(zcat ",infiles[1]," | cut -d \" \" -f1) <(zcat ",infiles[2]," | cut -d \" \" -f1) > ",outfile) # Changed cut delimiter
    } else {
      cmd = paste0("sort -um <(zcat ",infiles[1]," | cut -d \" \" -f1) <(zcat ",infiles[2]," | cut -d \" \" -f1)",
        paste0(" | sort -um - <(zcat ",infiles[3:length(infiles)]," | cut -d \" \" -f1)",collapse=""),
